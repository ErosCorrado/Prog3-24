...
export default class Login extends Component {
  ...
  onSubmit(email, password) {
    if (email === null || email === "" || !email.includes("@")) {
      this.setState({
        errors: { errorMail: 'Verifica que el correo electrónico sea válido' },
        loading: false
      });
      return false;
    }
    if (password === null || password === "" || password.length < 6) {
      this.setState({
        errors: { errorPassword: 'La contraseña no puede estar vacía y debe tener más de 6 caracteres' },
        loading: false
      });
      return false;
    }
    this.setState({ loading: true, errors: { errorMail: '', errorPassword: '' } });
    ...
    auth.signInWithEmailAndPassword(email, password)
      ...
      .catch(err => {
        ...
        this.setState({
          errors: { errorMail: '', errorPassword: '' },
          loading: false,
        });
    ...
    return (
      <View style={styles.container}>
        ...
        <TextInput
          ...
          onChangeText={(text) => this.setState({ email: text, errors: { ...this.state.errors, errorMail: '' } })}
        ...
        />
        {this.state.errors.errorPassword !== '' && <Text style={styles.errorText}>{this.state.errors.errorPassword}</Text>}
